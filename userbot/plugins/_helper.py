...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "@Dark_shadow_support_group"
            ...
            if len(string) > 69:
                with io.BytesIO(str.encode(string)) as out_file:
                    ...
                    await bot.send_file(
                        ...
                        force_document=True,
                        allow_cache=False,
                        caption="**COMMANDS** In DARK SHADOW",
...
@borg.on(admin_cmd(pattern="ds"))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
...
async def _(event):
    ...
    await event.edit("Telethon UserBot powered by DarkShadow")
